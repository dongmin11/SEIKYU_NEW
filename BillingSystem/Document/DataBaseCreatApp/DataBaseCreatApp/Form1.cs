...
namespace DataBaseCreatApp
{
    public partial class Form1 : Form
    {
        ...
        {
            dateTime = "SQL" + DateTime.Now.ToString("yyyyMMddhhmmss");
        ...
        private List<string> CreatTableSql(ISheet sheet)
        {
            ...
            for (int rowNo = 9; rowNo < 999; rowNo++)
            {
                ...
                if (pkFlg == "○")
                {
                    if (premaryKeyStr != "") premaryKeyStr += " , ";
                    premaryKeyStr += "[" + colunmKey + "] ASC";
                }
            ...
            if (premaryKeyStr != "")
            {
                sqlStr.Add("	CONSTRAINT PK_" + tableNm + " PRIMARY KEY CLUSTERED (" + premaryKeyStr + ") WITH (");
        ...
        private void CreatSqlFile(string fileNm, List<string> tableSql)
        {
            ...
            {
                if (!System.IO.Directory.Exists("./" + dateTime))
                {
                    Directory.CreateDirectory("./" + dateTime);
                }
                Encoding enc = Encoding.GetEncoding("UTF-8");
                using (StreamWriter writer = new StreamWriter("./"+ dateTime + "/Creat_" + fileNm +".txt", false, enc))
                {
